  ...
  return (
    <aside className={`mission-queue ${open ? 'is-open' : ''}`} aria-hidden={!open}>
      ...
      <div className="queue-list">
        {queue.length === 0 ? <div className="queue-empty">{t('queue.empty')}<br />{t('queue.emptyDetail')}</div> : queue.map((task, index) => (
          <article className="queue-record" key={task.id}>
            <span className="queue-position">{(index + 1).toString().padStart(2, '0')}</span>
            <div><strong>{task.profileId.replace(/-/g, ' ').toUpperCase()}</strong><p>{typeof task.input.objective === 'string' ? task.input.objective : 'Bounded agent task'}</p><small>{task.options.priority.toUpperCase()} PRIORITY</small></div>
