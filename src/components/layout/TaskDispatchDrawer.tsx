...
export function TaskDispatchDrawer({ open, profiles, profilesLoading = false, extensionRegistry = profileExtensionRegistry, onClose, onDispatch }: TaskDispatchDrawerProps) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    onDispatch({
      ...
      input: Object.fromEntries(Object.entries(values).map(([key, value]) => [key, value.trim()])),
  ...
  return <aside className="task-dispatch-drawer open"><div className="dispatch-heading"><span>{t('dispatch.title')}<small>{t('dispatch.draft')}</small></span><button type="button" onClick={onClose}>{t('dispatch.close')}</button></div><form onSubmit={submit}>
    ...
    {objectiveField && <><label>{objectiveField.label}<textarea className={validationAttempted && !(values.objective ?? '').trim() ? 'is-invalid' : undefined} value={values.objective ?? objectiveField.defaultValue} onChange={(event) => setValues((current) => ({ ...current, objective: event.target.value }))} rows={4} aria-invalid={validationAttempted && !(values.objective ?? '').trim()} aria-describedby="objective-guidance" /></label><small className={validationAttempted && !(values.objective ?? '').trim() ? 'objective-guidance is-error' : 'objective-guidance'} id="objective-guidance">{validationAttempted && !(values.objective ?? '').trim() ? t('dispatch.required') : objectiveField.description || t('dispatch.objectiveHint')}</small></>}
    {contextFields.length > 0 && <div className="dispatch-context"><span>{t('dispatch.input')}</span><div>{contextFields.map((field) => <label key={field.key}>{field.label}{field.widget === 'select' ? <select value={values[field.key] ?? field.defaultValue} onChange={(event) => setValues((current) => ({ ...current, [field.key]: event.target.value }))}>{field.options.map((option) => <option key={option} value={option}>{option}</option>)}</select> : <input className={validationAttempted && field.required && !(values[field.key] ?? '').trim() ? 'is-invalid' : undefined} value={values[field.key] ?? field.defaultValue} onChange={(event) => setValues((current) => ({ ...current, [field.key]: event.target.value }))} placeholder={field.placeholder} aria-invalid={validationAttempted && field.required && !(values[field.key] ?? '').trim()} />}</label>)}</div></div>}
